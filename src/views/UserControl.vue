      ...
      <div class="form-section">
        ...
        <form @submit.prevent>
          <div class="form-group">
            ...
            <input type="text" v-model="nome" class="form-control" autocomplete="off" />
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="password" v-model="senha" class="form-control" autocomplete="off" />
          </div>
          <div class="form-group">
            <label for="autorizacoes" class="form-label">Autorizações:</label>
            <select v-model="autorizacoes" class="form-control" multiple autocomplete="off">
              <option value="ROLE_USER">ROLE_GLOBAL</option>
              <option value="ROLE_ADMIN">ROLE_USER</option>
              <option value="ROLE_ADMIN">ROLE_ADMIN</option>
            </select>
          </div>
          <div class="button-group">
            <button @click="confirmAction(registerUser, 'Confirmar registro do Usuário?')" class="btn" :disabled="!isFormValid || Boolean(selectedUser)">Cadastrar</button>
            <button @click="confirmAction(updateUser, 'Confirmar atualização do Usuário?')" class="btn" :disabled="!isFormValid || !Boolean(selectedUser) || !isFormChanged">Atualizar</button>
...
export default {
  ...
  setup() {
    ...
    const registerUser = async () => {
      ...
      await axios.post('http://localhost:8080/cadastro', novoUsuario);
      await loadData();
      resetForm();
    };
    ...
    const selectUser = (usuario: Usuario) => {
      if (selectedUser.value && selectedUser.value.id === usuario.id) {
        selectedUser.value = null;
        resetForm();
      } else {
    ...
    const isFormChanged = computed(() => {
      return (
        selectedUser.value &&
        (nome.value !== selectedUser.value.nome ||
          email.value !== selectedUser.value.email ||
          senha.value !== selectedUser.value.senha ||
          autorizacoes.value.join(',') !== selectedUser.value.autorizacoes.map(auth => auth.nome).join(','))
      );
    });
